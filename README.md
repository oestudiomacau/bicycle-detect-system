...
这是根据开题报告搭建的未接入海康威视 SDK 的桌面端框架。系统支持模拟视频源、本地视频和本地图片，并使用 Hugging Face RT-DETR 检测自行车、电动车和摩托车。
...
- 工具栏手动截图（告警不会自动截图）
- 独立“事件与截图”页面，支持事件筛选、日志打开、截图列表与预览
- 海康 HCNetSDK、Anomalib 模型适配接口
- 三段开放许可示例视频和快速加载菜单
- 本地视频播放、真实两轮车检测和简单质心跟踪
- JPG、JPEG、PNG、BMP、WebP 图片检测
...
模拟源中的检测框由模拟引擎生成。本地视频和图片在已安装时优先使用 `PekingU/rtdetr_r50vd_coco_o365`，模型通过独立 CUDA 进程异步运行；加载期间或不可用时自动回退到 YOLOX-Tiny 和 MobileNet SSD。图片会先显示回退模型的快速结果，RT-DETR 就绪后再自动刷新高精度结果。速度结果仍依赖现场距离标定和稳定跟踪，停车模块当前只对真实检测框执行越界规则，倒地与异常堆放需要接入 Anomalib。
...
告警事件会写入 `runtime/events.jsonl`；只有点击工具栏“保存截图”时，图片才会保存到 `runtime/snapshots/`。
